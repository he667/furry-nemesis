...
public class ThemeManager {
	...
	public static void downloadThemes(Context applicationContext) {
		...
		try {
			...
			Log.d(FruityClockActivity.TAG, "Downloaded themes " + result);

		} catch (IOException e) {
			Log.e(FruityClockActivity.TAG, "Error IO Exception downloading theme", e);
		} finally {
	...
	private static void mergeTheme(Theme theme) {
		...
		Log.d(FruityClockActivity.TAG, "Checking theme preexistence " + theme.getTid());
		List<Entity> previous = DataFramework.getInstance().getEntityList("themes", "tid=" + theme.getTid());
		if (previous != null && previous.isEmpty()) {
			Log.d(FruityClockActivity.TAG, "inserting theme " + theme.getTid());
			theme.toEntity().save();
		}
	}
